...
## CASANDRE parameters
cruns         = 3   ## (10) CASANDRE runs per Ss
sample_rate   = 300   ## (100) higher values produce slower, more precise estimates
...
def make_param_combos(bounds, granularity = 2):
    """
        INPUT
        - bounds: bounds of each parameter
        - granularity: number of values you get within the domain. Evenly spaced, but does not include start/end.
        OUTPUT
        - params: every combination of boundary spaces, given granularity
    """
    np = numpy
    params = None
    multi = 1
    for bound in bounds:
        step = (bound[1] - bound[0]) / (granularity + 1) ## size of the step, given boundary range
        tack = np.arange(bound[0], bound[1] + step / 2, step)[1:-1].reshape(-1, 1) ## range excluding start/end, reshaped into column
        if params is None: ## keep previous tack column
            params = tack
        else: ## get every combination of old values with new column
            params = np.hstack((np.vstack((params, ) * granularity), np.vstack((tack, ) * multi)))
        multi *= granularity ## multiply by number of steps, so next iteration tack knows how many to repeat
    return params
...
def sub_func(idx, sub): ## iterate through Ss IDs
    ...
    combos = make_param_combos(bounds, 2)
    nll = np.array([neg_ll(params, stim_vals, math_choices, all_run_idx, all_contrast_idx, nll_counter) for params in combos])
    idx_sort = np.argsort(nll)
    best_start_params = [params for params in combos[idx_sort][:cruns]]
    best_param_est = best_start_params[0]
    best_nll = nll[idx_sort][0]
    for crun, search_start in enumerate(best_start_params): ## CASANDRE runs
        ...
        if estimate.fun < best_nll: ## comparing estimates
...
def time_negll(idx, sub): ## used for optimization debugging
    np = numpy
    pd = pandas
    if idx + 1 <= skip_num: ## checks if current Ss has already been run
        return ## go to next Ss
    ...
    combos = make_param_combos(bounds, 2)
    t0 = time.time()
    nll = neg_ll(combos[0], stim_vals, math_choices, all_run_idx, all_contrast_idx, nll_counter)
    print(time.time() - t0)
...
if os.path.exists(join(save_proj_dir, "{0}-cpdm_best-params-est{1}.csv".format(project_name.lower(), version))): ## checks for previous best params file
    existing_df          = pd.read_csv(join(save_proj_dir, "{0}-cpdm_best-params-est{1}.csv".format(project_name.lower(), version))) ## make new Ss save directory
...
time_negll(0, "0001") ##optimization debugging
...
with ipp.Cluster(n = n_cores) as rc: ## 14 cores (starts and connects to the local cluster)
    ...
    rc[:].push(dict(casandre_fit = casandre_fit, ## pushed functions
                    _logninv = _logninv,
                    logninv = logninv,
                    make_param_combos = make_param_combos,
                    neg_ll = neg_ll, 
                    normcdf = stats.norm.cdf,
    ...
    for batch in range(batches):
        ...
        if new_results == True:
            ...
            estimate_df.to_csv(join(save_proj_dir, "{0}-cpdm_best-params-est{1}.csv".format(project_name.lower(), version)), index = False) ## save completed dataframe w/ CRDM alpha
