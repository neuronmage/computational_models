...
## CASANDRE parameters
cruns         = 1   ## (10) CASANDRE runs per Ss
...
def sub_func(idx, sub, all_sub_params): ## iterate through Ss IDs
    np = numpy
    pd = pandas
    ...
    best_param_est = all_sub_params[idx] ## set empty best params variable
    all_choice_llhs, actual_choice_llhs, max_choice_llhs = neg_ll(best_param_est, stim_vals, math_choices, all_run_idx, 
                                                                    all_contrast_idx, nll_counter, output_llhs = True)
...
if project_name == "RIC1":
    ...
    run_labels = dict(low_vol_no_risk = "LVNR", low_vol_low_risk = "LVLR", low_vol_high_risk = "LVHR")

files = sorted(glob(os.path.join(data_dir, "2*_IDM_*.csv")))
...
if os.path.exists(join(base_proj_dir, "derivatives/{0}/parameter_estimation/cpdm/bestParamEstCombAlpha.csv").format(project_name)):
    existing_df = pd.read_csv(join(base_proj_dir, "derivatives/{0}/parameter_estimation/cpdm/bestParamEstCombAlpha.csv").format(project_name)) 
    skip_num = 0 
    existing_df["High Risk MU"] = existing_df["uncMeta_3"]
    existing_df["Low Risk MU"] = existing_df["uncMeta_1"]
    existing_df["No Risk MU"] = existing_df["uncMeta_2"]
    existing_df["High Risk CC"] = existing_df["confCrit_3"]
    existing_df["Low Risk CC"] = existing_df["confCrit_1"]
    existing_df["No Risk CC"] = existing_df["confCrit_2"]
    for x in range(1, 4):
        del existing_df["uncMeta_{0}".format(x)]
        del existing_df["confCrit_{0}".format(x)]
    del existing_df["crdm_alpha"]
    all_sub_params = [np.array(row) for row in existing_df.values.tolist()]

with ipp.Cluster(n = n_cores) as rc: ## 12 cores (starts and connects to local cluster)
    ...
    for batch in range(batches):
        ...
        if batch == batches - 1:
            cap = unclean_batch
        res = rc[:cap].map_sync(sub_func, 
                                range(batch * n_cores, batch * n_cores + cap), 
                                subs[batch * n_cores:batch * n_cores + cap], 
                                [all_sub_params] * cap)
        new_results = False
        for i, returned_params in enumerate(res):
            ...
            for curr_run, _ in enumerate(np.unique(all_choice_llhs[:, 0])):
                curr_run_idx = np.where(all_choice_llhs[:, 0] == curr_run + 1)
            ...
            curr_sub = subs[batch * n_cores:batch * n_cores + cap][i]
            sub_choices_llh = all_choice_llhs
            all_sub_params[batch * n_cores + i] = np.hstack((curr_sub, best_param_est))
        batch_end_time = time.time()
        print(" | Last batch runtime: {0:03d}".format(int(batch_end_time - batch_start_time)), end = "\r")
all_sub_params2 = np.vstack(all_sub_params)
crdm_params_file = os.path.join(base_proj_dir, 
                                "derivatives/{0}/parameter_estimation/crdm/softmax/{1}-crdm-sm_modelfit.csv".format(project_name, project_name.lower()))
crdm_params_df = pd.read_csv(crdm_params_file)
estimate_df = pd.DataFrame(all_sub_params2, columns = column_names)
estimate_df["subID"] = estimate_df["subID"].apply('="{}"'.format)
estimate_df["CRDM Alpha"] = crdm_params_df["Alpha"]
estimate_df.to_csv(join(save_proj_dir, "{0}-cpdm_MATLAB_casandre-ACC.csv".format(project_name.lower())), index = False)